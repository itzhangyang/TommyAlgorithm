...
### 问题的本质 - 哪些节点可以同时通往左边界和上边界

我们现在知道$grid[row][col]$代表了当前格子的高度。

如果$grid[row][col]$能够找到一条路径，在这条路径中

- 后一个节点总是比前一个节点的高度低
- 这条路径最终可以到达左边界($grid[0..m-1][0]$) 中的某一个节点 和 上边界($grid[0][0...n-1]$)中的某一个节点。

###核心思想-利用多源BFS反向遍历

我们如果对所有节点逐个验证其是否能够到达上边界和左边界，将会比较耗时。

相反，我们利用**多源BFS**分别从上边界和左边界的任意节点出发。
